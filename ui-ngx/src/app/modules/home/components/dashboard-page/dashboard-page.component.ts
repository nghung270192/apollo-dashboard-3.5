...
export class DashboardPageComponent extends PageComponent implements IDashboardController, OnInit, AfterViewInit, OnDestroy {
  ...
  editWidgetsTypesToDisplay($event: Event) {
    ...
    const widgetTypesList = Array.from(this.dashboardWidgetSelectComponent.widgetTypes.values()).map(type => ({type, display: filterWidgetTypes === null ? true : filterWidgetTypes.includes(type)}));
