...
@Component({
  selector: 'tb-add-widget-to-dashboard-dialog-controller',
      ...
      this.dashboardService.getDashboard(dashboardId).pipe(
        mergeMap((dashboard) => {
          ...
          return this.selectTargetState(dashboard).pipe(
            mergeMap((targetState) => forkJoin([of(dashboard), of(targetState), this.selectTargetLayout(dashboard, targetState)]))
          );
        })
      ).subscribe((res) => {
