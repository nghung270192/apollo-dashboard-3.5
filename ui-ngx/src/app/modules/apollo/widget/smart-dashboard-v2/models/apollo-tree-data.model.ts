...
import {NodeTreeTypeLabelMapping} from '@modules/apollo/widget/smart-dashboard-v2/models/apollo-entity-type.model';
  ...
  getAllNodesAndFilter(pageLink: PageLink, type: string = '', model: string = ''): Observable<PageData<NodeTree>> {
    ...
    if (pageLink) {
      ...
      if (pageLink?.sortOrder) {
        dataFilter = dataFilter.sort((a, b) => sortItems(a, b, pageLink?.sortOrder.property,
          pageLink?.sortOrder.direction === Direction.ASC));
      }
