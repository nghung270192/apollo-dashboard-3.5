...
export class MapViewComponent implements OnInit, OnChanges, OnDestroy, AfterViewInit {
  ...
  private initOpenStreetMap() {
    ...
    const nodeTree = this.nodeTrees.find(value => {
      if (value?.additionalInfo?.mapStreetLocation) {
        return value;
      }
    });
    ...
    this.nodeTrees.map(value => {
      let location = defaultCenterLocationOpenStreetMap;
      let label = defaultCenterLocationOpenStreetMap.label;
      if (value?.additionalInfo?.mapStreetLocation) {
        location = value?.additionalInfo?.mapStreetLocation;
      }
      /*      if (entity?.mapLabel)
              label = entity.mapLabel;
            else*/
      label = value.name.slice(0, 20) + (value.name.length > 20 ? '...' : '');

      const maker = L.marker([location.lat, location.lng], {
        icon: L.icon({
          iconUrl: value?.additionalInfo?.iconOnMap ? value?.additionalInfo?.iconOnMap : './assets/apollo/icon/map-place.png',
        ...
        .on('pm:dragend', (e) => {
          if (e.shape === 'Marker') { // @ts-ignore
            ...
            value.additionalInfo.mapStreetLocation = location;
            this.saveNodeTree(value);
          }
          ...
          if (!this.isDrag) {
            this.entityDoubleClick.emit(value);
          }
  ...
  private updateBounds(updateImage?: boolean, lastCenterPos?: any) {
    const w = this.width;
    const h = this.height;
    const southWest = this.pointToLatLng(0, h);
    const northEast = this.pointToLatLng(w, 0);
    ...
    const southWest1 = this.pointToLatLng(-padding, h + padding);
    const northEast1 = this.pointToLatLng(w + padding, -padding);
  ...
  private loadImageMap(): void {
    ...
    this.nodeTrees.forEach(nodeTree => {
      let location: MapLocation = {
        lng: this.pointToLatLng(0, 0).lng,
        lat: this.pointToLatLng(0, 0).lat,
        label: ''
      };
      let label = '';
  ...
  setting() {
    ...
    let sub: any;
    sub = this.dialog.open(AreaSettingComponent, dialogConfig).afterClosed();
    if (sub) {
      sub.subscribe(res => {
        if (res && res?.data) {
